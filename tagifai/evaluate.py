...
# ------------------------------ Different evaluate functions for different situations ------------------------

@slicing_function()
def nlp_cnn(x):
    """NLP projects that uses convolution."""
...
@slicing_function()
def short_text(x):
    """Projects with short title and descriptions."""
...
def get_slice_metrics(y_true, y_pred, slices):
    ...
    for slice_name in slices.dtype.names:
        ...
        if sum(mask):
            slice_metrics = precision_recall_fscore_support(y_true[mask], y_pred[mask], average="micro")
